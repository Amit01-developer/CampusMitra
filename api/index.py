...
_cred_path = os.environ.get('FIREBASE_CREDENTIALS', str(pathlib.Path(__file__).parent.parent / 'backend' / 'serviceAccountKey.json'))
...
def authHeaders():
    return {'Authorization': f'Bearer {request.headers.get("Authorization", "")}'}
...
def google_login():
    ...
    if existing:
        ...
        udata = {
            ...
            'department': '', 'year': '', 'campus_zone': '',
            'trust_score': 5.0, 'is_verified': True,
            'created_at': datetime.utcnow().isoformat(),
        }
...
def get_categories():
    ...
    for doc in docs:
        ...
        result.append({
            'id': doc.id, 'slug': d.get('slug'), 'name': d.get('name'),
            'description': d.get('description'), 'icon': d.get('icon'), 'color': d.get('color'),
            'stats': {
...
def create_item():
    ...
    item_data = {
        ...
        'price': data['price'], 'price_amount': data['price_amount'],
        'price_unit': data.get('price_unit', 'day'), 'condition': data.get('condition', 'Good'),
        'deposit': data.get('deposit', ''), 'deposit_amount': data.get('deposit_amount'),
        'is_available': True,
        'campus_zone': data.get('campus_zone', user.get('campus_zone', '')),
        'category_slug': data['category_slug'], 'owner_id': user['id'],
        'owner': {
            'id': user['id'], 'name': user.get('name'),
            'department': user.get('department'), 'trust_score': user.get('trust_score', 5.0),
        },
        'created_at': datetime.utcnow().isoformat(),
    }
...
def create_rental():
    ...
    rental_data = {
        'item_id': item_doc.id, 'borrower_id': user['id'],
        'lender_id': item_data.get('owner_id'), 'rental_type': data.get('rental_type', 'rent'),
        'status': 'pending', 'start_date': data.get('start_date'),
        'end_date': data.get('end_date'), 'total_price': data.get('total_price'),
        'created_at': datetime.utcnow().isoformat(),
...
def get_stats():
    ...
    return jsonify({
        ...
        'total_rentals': total_rentals, 'savings': total_rentals * 350, 'satisfaction': 90,
    })
