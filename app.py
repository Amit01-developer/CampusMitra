...
def google_login():
    ...
    try:
        # clock_skew_seconds=10 handles minor clock drift between server and Google
        ...
        print(f'[google_login] InvalidIdTokenError: {e}')
        return jsonify({'error': 'Invalid Google token', 'detail': str(e)}), 401
    except Exception as e:
        print(f'[google_login] Exception: {type(e).__name__}: {e}')
        return jsonify({'error': 'Google login failed', 'detail': str(e)}), 401
...
def get_items():
    ...
    condition = request.args.get('condition')
    # available=true  → only available items (default for browse)
    # available=false → ALL items regardless of availability (for owner dashboard)
